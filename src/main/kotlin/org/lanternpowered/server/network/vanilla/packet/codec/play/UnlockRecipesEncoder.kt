...
object UnlockRecipesEncoder : PacketEncoder<UnlockRecipesPacket> {
    ...
    override fun encode(ctx: CodecContext, packet: UnlockRecipesPacket): ByteBuffer {
        val buf = ctx.byteBufAlloc().buffer()
        val action = when (packet) {
            is UnlockRecipesPacket.Init -> 0
        ...
        if (packet is UnlockRecipesPacket.Init) {
