    ...
    private fun toLegacy(builder: StringBuilder, locale: Locale, text: Text, formattingCode: Char, baseStyle: Style, appliedStyle: Style): StringBuilder {
        ...
        if (formattingCode.toInt() != 0) {
            ...
            if (appliedStyle.color != null && newStyle.color == null ||
                ...
                builder.append(formattingCode).append(TextConstants.RESET)
                if (newStyle.color != null)
                    builder.append(formattingCode).append(TextColorRegistry.getCode(newStyle.color!!))
                if (newStyle.bold)
                    builder.append(formattingCode).append(TextConstants.BOLD)
                if (newStyle.italic)
                    builder.append(formattingCode).append(TextConstants.ITALIC)
                if (newStyle.obfuscated)
                    builder.append(formattingCode).append(TextConstants.OBFUSCATED)
                if (newStyle.underlined)
                    builder.append(formattingCode).append(TextConstants.UNDERLINE)
                if (newStyle.strikethrough)
                    builder.append(formattingCode).append(TextConstants.STRIKETHROUGH)
            } else {
        ...
        if (text is LiteralText) {
            ...
            for (i in args0.indices) {
                var `object` = args[i]
                if (`object` is TextRepresentable) {
                    if (`object` is Text) {
                        // Ignore
                    } else if (`object` is Text.Builder) {
                        `object` = `object`.build()
                    } else {
                        `object` = `object`.toText()
                    }
                    args0[i] = toLegacy(StringBuilder(), locale, `object` as Text, formattingCode, baseStyle, appliedStyle).toString()
                } else {
                    args0[i] = `object`
                }
            }
