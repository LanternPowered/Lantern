...
class StatusPingCodec : Codec<StatusPingMessage> {
    ...
    override fun encode(context: CodecContext, message: StatusPingMessage): ByteBuffer =
            context.byteBufAlloc().buffer(Long.SIZE_BYTES).writeLong(message.time)

    override fun decode(context: CodecContext, buf: ByteBuffer): StatusPingMessage = StatusPingMessage(buf.readLong())
}
