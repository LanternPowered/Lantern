...
public final class HandlerHandshakeIn implements Handler<HandshakePacket> {
    ...
    @NettyThreadOnly
    @Override
    public void handle(NetworkContext context, HandshakePacket packet) {
        final Optional<ProtocolState> optNextState = ProtocolState.getFromId(packet.getNextState());
        final NetworkSession session = context.getSession();
        if (!optNextState.isPresent()) {
            session.close(t("Unknown protocol state! (%s)", packet.getNextState()));
            return;
        }
        ...
        switch (proxyType) {
            ...
            case VELOCITY:
                ...
                if (split.length == 3 || split.length == 4) {
                    virtualAddress = new InetSocketAddress(split[1], packet.getPort());
            ...
            case NONE:
                ...
                virtualAddress = new InetSocketAddress(hostname, packet.getPort());
                break;
            default:
